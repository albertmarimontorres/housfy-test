<template>
  <div class="rentals-view">
    ...
    <v-card v-else-if="!rentalStore.hasRentals" class="text-center py-12" variant="outlined">
      <v-card-text>
        <v-icon size="80" color="grey" class="mb-4"> mdi-home-variant-outline </v-icon>
    ...
    <div v-else>
      <div class="d-flex align-center justify-between mb-4">
        <span class="text-body-1">
          <strong>{{ rentalStore.rentalsCount }}</strong> alquiler{{
            rentalStore.rentalsCount !== 1 ? 'es' : ''
          }}
...
const RentalCard = defineAsyncComponent({
  loader: () =>
    import(/* webpackChunkName: "rental-components" */ '@/components/domain/RentalCard.vue'),
  loadingComponent: {
    template: `
    ...
    async refreshRentals() {
      ...
      if (
        Object.keys(this.filters).some(
          key => this.filters[key as keyof RentalFilters] !== undefined
        )
      ) {
        this.applyFilters();
      }
